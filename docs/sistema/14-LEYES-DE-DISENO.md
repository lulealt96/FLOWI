...
Transición entre pantallas:  300-400ms | ease-out (entra) / ease-in (sale)
...
- **`ease-in` casi nunca en UI.** Sirve para algo que "se va y no vuelve" (raro). En la práctica, si dudas, `ease-out`.
- **Nota de mantenimiento del SO:** la tabla de arriba (y `10`) asignan `ease-in` a *salidas de pantalla* ("ease-in (sale)"). Emil Kowalski recomienda `ease-out` también para salidas. Tratar `ease-out` como default tanto para entradas como salidas de UI; reservar `ease-in` para casos deliberados.
...
--ease-out: cubic-bezier(0.16, 1, 0.3, 1);     /* entradas, el workhorse */
