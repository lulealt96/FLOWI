...
promesa como si la app existiera, con un CTA "Pre-ordenar" / "Quiero acceso anticipado":
  ...
  - Mandar tráfico mínimo (orgánico propio, un grupo de nicho, $20-50 de ads).
SEÑAL QUE PASA EL GATE: ≥ 3-5 pre-órdenes pagadas REALES (o ≥ 10 si el tráfico fue amplio).
  Un clic en "comprar" que llega al checkout (intención de pago) cuenta como señal media;
  el pago consumado es la señal fuerte. Reembolsar después si la app aún no existe.
SEÑAL QUE NO PASA: muchos "me interesa" y cero que llegan al checkout → el mensaje o la
  demanda fallan. Iterar el ángulo o pivotar (NO construir).
```
...
Señal obtenida:      [ej. "4 pre-órdenes pagadas" | "4/5 nombran $15-25 y describen el dolor"]
Umbral exigido:      [ A: ≥3-5 pagos / ≥10 con tráfico amplio  |  B: ≥4/5 con WTP viable ]
...
```
1. VINO POR FLUJO A (arbitraje LATAM, INICIO.md): el GATE DE DEMANDA se considera CUBIERTO por la
   evidencia de mercado de la oportunidad (app que ya factura afuera + dolor confirmado en LATAM +
   brecha en español). NO se exige pre-venta ni 5 entrevistas. Documentar en ESTADO.md "demanda
   validada por arbitraje (FLUJO A)". → Igual SÍ se corre el GATE DE VIABILIDAD UNITARIA (Paso 3 + 40):
   que alguien quiera ≠ que el negocio gane dinero.
...
- [ ] **GATE DE DEMANDA pasado (Paso 0): señal de pago real (≥3-5 pre-órdenes) o ≥4/5 entrevistas con WTP viable — SIN esto no se construye**
