...
2. SNAPSHOT cuando el cambio está bien:
   `supabase db pull <nombre_descriptivo> --local`
   → genera el archivo de migración limpio a partir del estado real.

3. AUDITAR antes de commitear:
   `supabase db advisors`  (o get_advisors vía MCP)
   → detecta automáticamente índices faltantes, RLS desactivado, y problemas de seguridad.
...
MIGRACIONES
[ ] Generadas con `supabase db pull` (flujo declarativo), no a mano sobre producción
[ ] `supabase db advisors` sin alertas críticas
