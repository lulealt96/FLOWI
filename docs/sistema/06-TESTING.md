...
[ ] El usuario puede hacer su primera acción sin registrarse (si aplica)
[ ] El onboarding se completa correctamente
[ ] La pantalla core muestra el input y el botón de acción
...
[ ] Safari (el más problemático — probar en iPhone o macOS)
    ...
    - CSS gap en flexbox (no soportado en Safari <14.1)
...
for (const [nombre, ancho] of [['mobile', 375], ['tablet', 768], ['desktop', 1440]] as const) {
  test(`pantalla core sin regresión visual @${nombre}`, async ({ page }) => {
    await page.setViewportSize({ width: ancho, height: 900 });
    await page.goto('/app');
    await expect(page).toHaveScreenshot(`core-${nombre}.png`, { maxDiffPixelRatio: 0.01 });
  });
}
```
...
for (const ruta of ['/', '/app', '/pricing']) {
  test(`sin violaciones de accesibilidad serias @${ruta}`, async ({ page }) => {
    ...
    const { violations } = await new AxeBuilder({ page })
      .withTags(['wcag2a', 'wcag2aa', 'wcag21a', 'wcag21aa']) // estándar que exige el SO (Test 5)
      .analyze();
