...
> **Siembra los eventos AL CONSTRUIR, no en una "sesión de analítica" al final.** El error de secuencia típico: dejar toda la instrumentación para la Sesión 7. Pero los eventos de activación y retención (`aha_alcanzado`, `sesion_iniciada`, `event_log`) nacen pegados a la pantalla que los produce — el `aha_alcanzado` se emite donde ocurre la primera victoria (Sesión 3), no se "añade después". Regla: cuando construyas la pantalla de una acción de valor, instrumenta su evento ahí mismo. La Sesión 7 (con 34) arma los funnels, la atribución por canal y los dashboards SOBRE eventos que ya existen — no los crea desde cero.
...
export function initAnalytics() {
  ...
  posthog.init(process.env.NEXT_PUBLIC_POSTHOG_KEY!, {
    api_host: process.env.NEXT_PUBLIC_POSTHOG_HOST,  // host EXPLÍCITO: us (app.posthog.com) o eu
                                                     // (eu.posthog.com) — define dónde residen los
