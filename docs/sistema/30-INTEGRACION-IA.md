# INTEGRACIÓN DE IA — Texto, Imagen, Audio y Resiliencia a Escala
...
ASÍNCRONO (job en background + notificar al terminar) — OBLIGATORIO para:
  → Generación de IMAGEN y AUDIO (tardan y/o producen archivos).
...
## IMAGEN (Nano Banana / Gemini 2.5 Flash Image) — asíncrono + storage
...
- API: gemini-2.5-flash-image ("Nano Banana", Google). ~$0.039 por imagen (≤1024×1024px,
  facturado por tokens de salida). Alternativas: DALL·E, Ideogram, Flux.
- Una imagen cuesta ~40× una generación de texto barata → fair-use ESTRICTO (ver economía).
...
create policy "own_jobs" on media_jobs for all using ((select auth.uid()) = user_id);
```
...
[ ] Decisión sync/async tomada: texto corto = streaming; imagen/audio = job asíncrono
