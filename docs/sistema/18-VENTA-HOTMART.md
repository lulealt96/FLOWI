...
MODELO 2 — ONBOARDING-FIRST / FREE TIER (registro gratis → onboarding/preview → paywall en la app):
  El usuario se AUTO-REGISTRA gratis con Supabase Auth (ver 26-AUTH-MODERNO) ANTES de pagar — su
  cuenta y su progreso del onboarding ya existen. Cuando compra (vía un link de checkout de Hotmart
  desde dentro de la app), el webhook lo ENCUENTRA por email y lo SUBE a Pro (rama `existing → update`).
  Es el caso de IA creativa, bienestar, fitness (preview/plan antes de pagar — ver matriz de 02B).
```

> ⚠️ **EL BUG A EVITAR (caveat crítico del Modelo 2): el match es por EMAIL.** Si el usuario se registró gratis con `caro@gmail.com` pero paga en Hotmart con `caro@hotmail.com`, el webhook NO lo encuentra → **crea una SEGUNDA cuenta Pro** y su cuenta gratis (con todo su onboarding/progreso) queda **huérfana**. El usuario entra a la cuenta vieja, la ve sin Pro, y cree que pagó en vano. Es un bug de pago real y silencioso. Mitigaciones (aplicar al menos las dos primeras):
>
> ```
...
> Cross-ref: la decisión Modelo 1 vs 2 se toma en `02B-ONBOARDING-MONETIZACION.md` (hard paywall vs onboarding-first) y el registro gratis del Modelo 2 usa el auth de `26-AUTH-MODERNO.md`.
...
6. SOLO MODELO 2: registrarse GRATIS primero con un email, LUEGO comprar con EL MISMO email →
...
// (verificar) el nombre EXACTO del evento/flag de inicio de trial en TU cuenta de Hotmart.
// Suele venir como estado de suscripción 'started' o un flag de trial en el payload.
...
[ ] SOLO MODELO 2: registro gratis + compra con el MISMO email → la cuenta se SUBE (no se duplica)
[ ] SOLO MODELO 2: mitigación del email distinto aplicada (pre-rellenar email + matchear por id/src)
