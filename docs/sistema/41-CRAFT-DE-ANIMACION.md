...
> **Nota de corrección al SO:** `14` (SISTEMA DE MOVIMIENTO) y `10` asignan `ease-in` a las salidas. Para movimiento de UI esto se siente lento. La recomendación de craft es usar `ease-out` también en salidas y reservar `ease-in` para casos de "sale y no vuelve". Revisar esa línea al pulir.
...
```css
/* Curvas de craft — más fuertes que las nativas. Conviven con los tokens de 14. */
...
> Estas son más agresivas que las de `14` (`cubic-bezier(0.16, 1, 0.3, 1)`). Ambas familias son válidas: la de `14` es el workhorse seguro; estas son para cuando quieres que el movimiento tenga FIRMA (ver motion signature en `16`, PASO 0.7). No mezclar tres curvas distintas en la misma pantalla sin razón.
...
La View Transitions API es **nativa del navegador**: no agrega bytes al bundle (casa perfecto con `38`). En Next App Router (ya **sin canary**) se usa con el componente `<ViewTransition>`, que llama a `startViewTransition` por debajo.
...
| **`14-LEYES-DE-DISENO.md`** | `14` tiene los **números exactos** (duraciones, las curvas nombradas `--ease-out`/`--ease-in`/`--ease-spring`) y el sistema de movimiento. `41` añade el porqué perceptual y **corrige** la asignación de `ease-in` a salidas. |
